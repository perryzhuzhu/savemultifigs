...
% Read Config file
cfg_file_default=fullfile('multifigs.ini');
cfg_file=fullfile(pwd,'multifigs.ini');
ini = IniConfig();
bConfig_exist=true;

if exist(cfg_file)
	ini.ReadFile(cfg_file);
elseif exist(cfg_file_default)
	ini.ReadFile(cfg_file_default);
else
    bConfig_exist=false;
end

if bConfig_exist
    out_dir = ini.GetValues('basic','out_dir');
    sub_dir = ini.GetValues('basic','sub_dir');
    fig_types = regexp(ini.GetValues('basic','fig_types'),',','split');
    save_types = regexp(ini.GetValues('basic','save_types'),',','split');
end
